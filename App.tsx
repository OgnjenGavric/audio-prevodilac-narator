    ...
    const handleStartRecording = async () => {
        ...
        try {
            ...
            mediaRecorderRef.current.onstop = () => {
                ...
                const recordedFile = new File([audioBlob], "snimak.wav", { type: mimeType });
            ...
            setError("Nije moguće pristupiti mikrofonu. Proverite dozvole u pretraživaču.");
            ...
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-10">
                    ...
                    <p className="mt-2 text-lg text-gray-400">
                        Snimite ili otpremite audio, dobijte transkripcije, prevode i AI-generisane glasovne naracije.
                    </p>
                </header>
                
                <main className="space-y-8">
                    <div className="bg-[#1E293B] shadow-2xl shadow-indigo-500/10 rounded-xl p-6 sm:p-8 w-full max-w-2xl mx-auto">
                        <h2 className="text-xl font-semibold mb-6 text-gray-200">1. Obezbedite Audio</h2>
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            <button 
                                onClick={isRecording ? handleStopRecording : handleStartRecording}
                                className={`w-full sm:w-auto flex-1 inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-md transition-all duration-300 ${isRecording ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'} text-white shadow-lg shadow-indigo-500/30`}
                            >
                            ...
                            <label htmlFor="file-upload" className={`w-full sm:w-auto flex-1 cursor-pointer inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-md bg-[#334155] ${isRecording ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#475569]'} text-gray-200 transition-colors duration-300`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                  <path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM6.293 6.707a1 1 0 010-1.414l3-3a1 1 0 011.414 0l3 3a1 1 0 01-1.414 1.414L11 5.414V13a1 1 0 11-2 0V5.414L7.707 6.707a1 1 0 01-1.414 0z" clipRule="evenodd" />
                                </svg>
                                Otpremi Fajl
                            </label>
                            <input id="file-upload" type="file" className="hidden" onChange={handleFileChange} accept="audio/*" disabled={isRecording} />
                        </div>
                         {audioFile && !isRecording && (
                             <p className="mt-4 text-center text-gray-400">Izabran fajl: <span className="font-medium text-indigo-400">{audioFile.name}</span></p>
                        )}
                        {isRecording && (
                             <p className="mt-4 text-center text-red-400 animate-pulse">Snimanje u toku...</p>
                        )}
                        ...
                        <div className="mt-6">
                            ...
                            >
                                {isLoadingTranscription ? <Loader size="6" className="text-white" /> : 'Transkribuj Audio'}
                            </button>
                        </div>
                    </div>
                    ...
                    {transcription && (
                        <div className="bg-[#1E293B] shadow-2xl shadow-indigo-500/10 rounded-xl p-6 sm:p-8 w-full max-w-2xl mx-auto space-y-6">
                           ...
                           <div>
                                <h2 className="text-2xl font-semibold mb-3 text-gray-200">3. Izaberite Jezik za Prevod</h2>
                                <div className="relative">
                                    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full bg-[#334155] text-gray-200 font-semibold py-3 px-4 rounded-lg inline-flex items-center justify-between transition-colors hover:bg-[#475569]">
                                        <span>{selectedLanguage ? selectedLanguage.name : "Izaberite jezik"}</span>
                                        <svg className={`fill-current h-4 w-4 transform transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                                    </button>
                                    {isDropdownOpen && (
                                        <div className="absolute z-10 w-full mt-1 bg-[#334155] border border-gray-600 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                            {LANGUAGES.map(lang => (
                                                <div 
                                                    key={lang.code} 
                                                    className={`p-3 cursor-pointer ${selectedLanguage?.code === lang.code ? 'bg-indigo-600 text-white' : 'text-gray-200 hover:bg-indigo-600/30'}`} 
                                ...
                                >
                                    {isLoadingTranslation ? <Loader size="6" className="text-white" /> : 'Prevedi'}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {translations.length > 0 && (
                        <div className="w-full max-w-7xl mx-auto">
                           <h2 className="text-3xl font-bold mb-6 text-center text-white">4. Prevod i Glasovna Naracija</h2>
